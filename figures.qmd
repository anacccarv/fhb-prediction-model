...
```{r}
p_compare <- ggplot(res, aes(pt)) +
  geom_ribbon(aes(ymin = NMB_lo, ymax = NMB_hi), alpha = 0.12, fill = "#4970b5") +
  geom_line(aes(y = NMB_mean), size = 1.2, color = "#ED7D31") +         # População
  geom_line(aes(y = NMB_trat_mean), size = 1.2, color = "#2CA02C", 
            linetype = "dashed") +                                      # Tratados
  geom_hline(yintercept = 0, linetype = 2, color = "gray40") +
  labs(
    x = expression("Risk threshold (" * italic(p)[italic(t)] * ")"),
    y = "Net Monetary Benefit (USD/ha)",
    caption = "Solid = population mean; Dashed = treated-unit mean"
  ) +
  theme_bw() +
  theme(
